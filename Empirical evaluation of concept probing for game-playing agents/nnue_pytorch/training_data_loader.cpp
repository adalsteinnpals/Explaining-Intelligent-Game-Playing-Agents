...
std::function<bool(const TrainingDataEntry&)> make_skip_predicate(bool filtered, int random_fen_skipping, bool wld_filtered, int early_fen_skipping, int param_index, int train_setting)
{
    if (filtered || random_fen_skipping || wld_filtered || early_fen_skipping)
    {
        return [
            ...
            // 7 = never only one with rook pair
            // 8 = never five pawns on dark squares
            ...
            // Return true if only one player has rook pair
            auto only_one_has_rook_pair = [&]() {
                return (e.blackHasRookPair() != e.whiteHasRookPair());
            };
            ...
            // Return true if total five pawns are on dark squares
            auto five_pawns_dark = [&]() {

                // print values to terminal
                //std::cout << "white pawns on dark squares: " << e.whitePawnsOnDarkSquares() << std::endl;
                //std::cout << "black pawns on dark squares: " << e.blackPawnsOnDarkSquares() << std::endl;
                //std::cout << "total pawns on dark squares-------: " << (e.whitePawnsOnDarkSquares() + e.blackPawnsOnDarkSquares() == 5) << std::endl;
                return e.whitePawnsOnDarkSquares() + e.blackPawnsOnDarkSquares() == 5;
            };
            ...
            if (train_setting == 7)
                if (only_one_has_rook_pair())
                    // print to terminal "throwing out using setting 7"
                    // std::cout << "throwing out using setting 7" << std::endl;
                    return true;

            if (train_setting == 8)
                if (five_pawns_dark())
                    // print to terminal "throwing out using setting 8"
                    // std::cout << "throwing out using setting 8" << std::endl;
                    return true;
