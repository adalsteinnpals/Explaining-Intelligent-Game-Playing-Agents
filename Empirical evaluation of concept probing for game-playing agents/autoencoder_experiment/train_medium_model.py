    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        with torch.no_grad():
            ...
            for batch in val_loader:
                # burn first max_iterations
                if it < max_iterations:
                    it += 1
                    continue
                print(f"Val: Epoch {epoch}, iteration {it}")

                if it == 2*max_iterations:
                    break
                it += 1
                ...
                print(f"Val: Epoch {epoch}, iteration {it}, loss: {loss.item()}")
            
            # Averaging out loss over entire batch
            running_loss_val /= max_iterations
